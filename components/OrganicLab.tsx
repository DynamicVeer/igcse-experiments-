...
const OrganicLab: React.FC = () => {
  ...
  const renderStructure = (id: string | null) => {
      if (!id) return null;
      ...
      switch (id) {
          ...
          case 'ethanol':
           ...
           default:
              return <text x="100" y="100" className={atomStyle}>{compound?.formula}</text>;
      }
  };

  // ... (Render Mechanism function remains same)
                                ...
                                {selectedOrganic ? (
                                    <div className="w-48 h-48 bg-slate-800/50 border border-slate-700 rounded-lg flex items-center justify-center relative backdrop-blur-sm">
                                        <svg width="200" height="200" viewBox="0 0 200 200" className="overflow-visible">
                                            {renderStructure(selectedOrganic)}
                                        </svg>
                                    </div>
                                ) : (
